 ...
 * placed without one").
 *
 * A person whose name is not known is drawn with #253's `lamp-unknown`: the dashed brass
 * perimeter with a lamp inside, which book-design-system.md calls for "in a register cameo or a
 * small medallion" - exactly this scale - rather than the full aala niche a scene-sized lamp gets.
 *
 * Budget: `diya` uses one gradient (its flame, by way of `diya-small`), and a gradient is the
 * single most expensive thing compose.js's artTerm counts (ART_PDF.gradients = 5800, against 1050
 * for a translucent layer and a fraction of a byte per path point), so a row's cost is dominated
 * by its lamp count almost regardless of how simple each lamp is. Measured (art-procedural.test
 * .mjs) at the design system's own page caps: a family page (<=8 people) is about 138 KB of
 * estimated PDF bytes, a gathering (<=12) about 207 KB. A page with many more people than that -
 * a cover lighting a whole family - should expect the cost to keep climbing roughly linearly
 * (measured at 48: about 828 KB) and budget its page count accordingly; this
