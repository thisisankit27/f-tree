...
class BookFormatTest {
    ...
    private fun golden() = File(repoRoot, "site/book/golden/sample-heirloom.json").readText()
    ...
    fun `an item this app does not know is refused`() {
        refusal(golden().replaceFirst("\"t\":\"rect\"", "\"t\":\"hologram\""))
    }

    @Test
    fun `an unknown key is refused`() {
        refusal(golden().replaceFirst("\"t\":\"rect\",", "\"t\":\"rect\",\"blend\":\"screen\","))
    }
    ...
    fun `a colour that is not rrggbb is refused`() {
        ...
        refusal(book.replaceFirst("\"fill\":\"$first\"", "\"fill\":\"red\""))
    }
    ...
    fun `the composer lays out a book from the app's own exported document`() {
        ...
        val script = """
            ...
            const template = JSON.parse(readFileSync('${File(repoRoot, "site/book/templates/heirloom.json").absolutePath}', 'utf8'));
            process.stdout.write(JSON.stringify(composeBook(doc, { now: '2026-09-15' }, template)));
        """.trimIndent()
        val book = readBook(runNode(script, input.absolutePath))
